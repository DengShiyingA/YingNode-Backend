...
def _write_file(path: Path, items):
    with _history_lock:
...
def load_history() -> List[Dict[str, Any]]:
    _ensure_file()
    try:
        return _read_file(HISTORY_FILE)
    except Exception:
        return []
...
def save_entry(host: str, status: str, username: str = '', note: str = '', nodes: Optional[List[Dict[str, Any]]] = None, error: Optional[str] = None, ports: Optional[List[str]] = None, validation: Optional[Dict[str, Any]] = None):
    ...
    items = items[:30]
    _write_file(HISTORY_FILE, items)
...
def load_servers():
    ...
    try:
        return _read_file(SERVERS_FILE)
    except Exception:
        return []
...
def save_server(host: str, username: str, note: str = '', password: str = ''):
    ...
    items = items[:20]
    _write_file(SERVERS_FILE, items)
