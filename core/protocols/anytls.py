...
Params schema (parallels Trojan):
- ``port`` (int)
- ``password`` (str)
- ``sni`` (str)
...
class AnyTlsProtocol(ProtocolSpec):
    name = "anytls"
    display_name = "AnyTLS"
    default_tag = "anytls-in"

    def build_inbound(self, params: Dict[str, Any]) -> Dict[str, Any]:
        for required in ("port", "password"):
            if not params.get(required):
                raise ValueError(f"anytls missing required param: {required}")
